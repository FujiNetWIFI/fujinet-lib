...
bool open_read_close_data(uint8_t cmd, int *bytes_read, uint16_t params_size, uint8_t *cmd_params, uint16_t result_size, uint8_t *result_data)
{
	...
	// doesn't seem to error when there's an error :(
	// printf("orcd wrote: %d, expected: %d, os: %02x\n", bytes_written, params_size, __oserror);
	if (bytes_written != params_size) {
		// write failed, we need to close the channel and do a status, but we do that anyway, so just set bytes_read to zero for now
		*bytes_read = 0;
	}
