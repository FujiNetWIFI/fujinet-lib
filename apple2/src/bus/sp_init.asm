                ...
                mcopy   macros/sp_init.macros

; C version 246 bytes, this is 104 or 110 bytes (65c02 vs 6502) as hand written asm

sp_init         start
                csubroutine ,5          5 bytes direct page space to store ptr1 & ptr2
                short m                 8-bit accumulator, 16-bit index registers

ptr1            equ     1               24-bit long ptr
ptr2            equ     4               16-bit ptr
...
; ptr2 = sp_detect_table, absolute addressing
	        ...
	        plb		        so can use absolute addressing
                ...
                ~MTStartup              startup Miscelaneous Tools
                ph2     #0
                ~MMStartup              startup Memory Manager
                pl2     MyID            save memory master id
                ph4     #0
                ~NewHandle #$500,MyID,#$c011,#0
                pl4     sp_handle
...
; clc - we know C is set because the previous cpy sets it in the comparison, so we can just add 2 here instead of 3 and save a byte
                adc     #$02
                sta     sp_dispatch_address
                lda     ptr1+1
                sta     sp_dispatch_address+1

; we set sp_is_init to stop sp_get_network_id from recursing
check_network   lda     #$01
                sta     sp_is_init

;        jsr     _sp_get_network_id             COMMENTED FOR UNIT TESTING

;        bne     found_network
                bra found_network               TESTING
